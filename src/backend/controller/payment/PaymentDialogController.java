...
public class PaymentDialogController implements Initializable {
	...
	private boolean validateForm() {
		...
		if ("Credit Card".equals(paymentMethod)) {
			return !cardNumberField.getText().trim().isEmpty() && !cvvField.getText().trim().isEmpty()
					&& !expiryDateField.getText().trim().isEmpty() && !cardHolderNameField.getText().trim().isEmpty();
	...
	private void processPayment() {
		...
		Task<PaymentResult> paymentTask = new Task<PaymentResult>() {
			...
			protected PaymentResult call() throws Exception {
				...
				return PaymentService.getInstance().initiatePayment(user, course, paymentMethod, cardNumber, cvv,
						expiry);
			}
