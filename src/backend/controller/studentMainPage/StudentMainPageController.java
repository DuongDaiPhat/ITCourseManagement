...
public class StudentMainPageController implements Initializable {
	...
	@FXML
	private FlowPane aiCoursesContainer;
	@FXML
	private FlowPane gameCoursesContainer;
	 ...
	 */
	private void updateContainerCourseStates(FlowPane container, java.util.Set<Integer> cartIds,
			java.util.Set<Integer> wishlistIds) {
		if (container == null)
			return;

		System.out
				.println("Updating container course states - Cart IDs: " + cartIds + ", Wishlist IDs: " + wishlistIds);
		...
		for (javafx.scene.Node node : container.getChildren()) {
			if (node instanceof VBox) {
				...
				if (courseId != null) {
					...
					System.out.println("Updating course " + courseId + " - In cart: " + isInCart + ", In wishlist: "
							+ isInWishlist);
						...
						if (infoNode instanceof HBox && infoNode.getStyleClass().contains("course-card-actions")) {
							...
							for (javafx.scene.Node actionNode : actionsContainer.getChildren()) {
								if (actionNode instanceof Button) {
									...
									if (btn.getStyleClass().contains("cart-icon-default")
											|| btn.getStyleClass().contains("cart-icon-active")) {
										cartButton = btn;
									} else if (btn.getStyleClass().contains("wishlist-icon-default")
											|| btn.getStyleClass().contains("wishlist-icon-active")) {
										wishlistButton = btn;
									}
								}
							}
	...
	private void setupNavigationEvents() {
		...
		categoryLabel.setOnMouseClicked(event -> {
			try {
				...
				SceneManager.switchSceneWithRefresh("Student Explore Page",
				...
				alert.setContentText("Could not navigate to the category page. Please try again.");
				alert.showAndWait();
			}
		});
	...
	private void loadCourses() {
		try {
			...
			if (allCourses != null && !allCourses.isEmpty()) { // Filter out purchased courses first
				...
				System.out.println("Loaded " + aiCourses.size() + " AI courses and " + gameCourses.size()
						+ " Game Development courses from database");
	...
	private List<Courses> createSampleCourses(String categoryName) {
		...
		if ("AI".equals(categoryName)) {
			String[] aiCourseNames = { "Introduction to Machine Learning", "Deep Learning Fundamentals",
			...
			String[] gamesCourseNames = { "Unity Game Development", "Unreal Engine Basics", "2D Game Programming",
	...
	private VBox createCourseCard(Courses course) {
		...
		titleLabel.setPrefHeight(40); // Fixed height for consistent layout // Course instructor (get from database
										// using UserService)
		...
		card.getChildren().addAll(imageView, titleLabel, instructorLabel, languageLevelBox, ratingLabel, priceLabel,
				actionsContainer);
	...
	private HBox createCourseCardActions(Courses course) {
		...
		wishlistButton.getStyleClass().addAll("course-card-icon", "wishlist-icon-default"); // Load icons (using
	...
	private void handleCartToggle(int userId, Courses course, Button cartButton) {
		try {
			...
			System.out.println("StudentMainPage: Cart toggle for course " + course.getCourseID()
			...
			if (isCurrentlyInCart) {
				...
				if (removed) {
					System.out
							.println("StudentMainPage: Successfully removed " + course.getCourseName() + " from cart"); // Force
																														// immediate
																														// UI
																														// update
																														// for
	...
	private void handleWishlistToggle(int userId, Courses course, Button wishlistButton) {
		try {
			...
			System.out.println("StudentMainPage: Wishlist toggle for course " + course.getCourseID()
			...
			if (isCurrentlyInWishlist) {
				...
				if (removed) {
					System.out.println(
							"StudentMainPage: Successfully removed " + course.getCourseName() + " from wishlist");
					// Force immediate UI update for this button
					javafx.application.Platform.runLater(() -> updateWishlistIconState(wishlistButton, false));
				} else {
					System.err
							.println("StudentMainPage: Failed to remove " + course.getCourseName() + " from wishlist");
				}
			} else {
				...
				if (added) {
					System.out
							.println("StudentMainPage: Successfully added " + course.getCourseName() + " to wishlist");
					// Force immediate UI update for this button
	...
	/**
	 * Refresh all course card states (cart and wishlist icons) to sync with current
	 * database state
	 */
	private void refreshCourseStates() {
		try {
			System.out.println("Refreshing StudentMainPage course states..."); // Reinitialize state manager to ensure
																				// sync with database
			StudentStateManager stateManager = StudentStateManager.getInstance();
			stateManager.initializeState();
